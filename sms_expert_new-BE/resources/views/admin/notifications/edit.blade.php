...
<main class="main-wrapper" id="main-wrapper">
    <div class="main-content">
        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3 breadcolor">
            <div class="breadcrumb-title pe-3 title-name">Edit Notification</div>
            <div class="ps-3">
                ...
                <div class="col-lg-8">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea name="message" class="form-control @error('message') is-invalid @enderror" 
                                              ...
                                              placeholder="Enter notification message" id="notificationMessage">{{ old('message', $notification->message) }}</textarea>
                                    <small class="text-muted"><span id="charCount">{{ strlen($notification->message) }}</span>/2000 characters</small>
                                    @error('message')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="type" class="form-select @error('type') is-invalid @enderror" id="notificationType">
                                        <option value="info" {{ old('type', $notification->type) == 'info' ? 'selected' : '' }}>ℹ️ Info</option>
                                        <option value="warning" {{ old('type', $notification->type) == 'warning' ? 'selected' : '' }}>⚠️ Warning</option>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="delivery_method" class="form-select @error('delivery_method') is-invalid @enderror">
                                        <option value="web" {{ old('delivery_method', $notification->delivery_method) == 'web' ? 'selected' : '' }}>🌐 Web Only</option>
                                        <option value="email" {{ old('delivery_method', $notification->delivery_method) == 'email' ? 'selected' : '' }}>📧 Email Only</option>
                                        <option value="mobile" {{ old('delivery_method', $notification->delivery_method) == 'mobile' ? 'selected' : '' }}>📱 Mobile Push Only</option>
                                        <option value="both" {{ old('delivery_method', $notification->delivery_method) == 'both' ? 'selected' : '' }}>🌐📧 Both (Email & Web)</option>
                                        <option value="web_mobile" {{ old('delivery_method', $notification->delivery_method) == 'web_mobile' ? 'selected' : '' }}>🌐📱 Both (Web & Mobile)</option>
                                        <option value="all" {{ old('delivery_method', $notification->delivery_method) == 'all' ? 'selected' : '' }}>🌐📧📱 All (Email, Web & Mobile)</option>
                                    </select>
                                ...
                                <div class="col-md-6 mb-3">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" name="requires_acknowledgment" id="requiresAck" 
                                               ...
                                               style="width: 45px; height: 22px;" value="1">
                    ...
                    <div class="card border-0 shadow-sm mb-4">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="target_type" id="targetAll" 
                                           value="all" {{ old('target_type', $notification->target_type) == 'all' ? 'checked' : '' }}>
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="target_type" id="targetSpecific" 
                                           value="specific" {{ old('target_type', $notification->target_type) == 'specific' ? 'checked' : '' }}>
                            ...
                            <div id="customerSelectContainer" class="{{ old('target_type', $notification->target_type) == 'specific' ? '' : 'd-none' }}">
                                ...
                                <select name="customer_ids[]" id="customerSelect" class="form-select" multiple>
                                    @foreach($customers as $customer)
                                        <option value="{{ $customer->id }}" 
                                                ...
                                                {{ in_array($customer->id, old('customer_ids', $selectedCustomerIds ?? [])) ? 'selected' : '' }}>
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="schedule_option" id="scheduleLater" value="later" 
                                           {{ !request('send_now') ? 'checked' : '' }}>
                            ...
                            <div id="scheduleDateContainer" class="{{ request('send_now') ? 'd-none' : '' }}">
                                ...
                                <small class="text-muted">Server time now: <strong id="serverTimeDisplay">{{ now()->format('d M Y, H:i:s') }}</strong> (UK Time). Schedule must be at least 2 minutes in future.</small>
                                @error('scheduled_at')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="card border-0 shadow-sm mb-4">
                        ...
                        <div class="card-body">
                            <p class="mb-2">
                                ...
                                <span class="badge bg-{{ $notification->status == 'sent' ? 'success' : ($notification->status == 'scheduled' ? 'info' : 'secondary') }}">
                    ...
                    <div class="card border-0 shadow-sm mb-4">
                        ...
                        <div class="card-body p-0">
                            <div class="preview-card">
                                <div class="preview-header {{ $notification->type }}" id="previewHeader">
                                    <h6 class="mb-0" id="previewTitle">{{ $notification->title }}</h6>
                                </div>
                                <div class="preview-body">
                                    <p class="mb-0" id="previewMessage">{!! nl2br(e($notification->message)) !!}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card border-0 shadow-sm position-sticky" style="top: 20px;">
                        <div class="card-body">
                            ...
                            <button type="submit" class="btn w-100 text-white mb-2" style="background: linear-gradient(135deg, #ea6118, #d1520e);">
                                ...
                                <span id="submitBtnText">Update Notification</span>
                            </button>
...
$(document).ready(function() {
    ...
    $('input[name="schedule_option"]').change(function() {
        if ($(this).val() === 'now') {
            ...
            $('#sendNowInput').val('1');
            $('#submitBtnText').text('Update & Send Now');
        } else {
    ...
    function updateSubmitButtonText() {
        ...
        if (scheduledAt) {
            $('#submitBtnText').text('Update & Schedule');
        } else {
            $('#submitBtnText').text('Update Notification');
        }
    }
